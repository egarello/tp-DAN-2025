...
export default function EditarReservaPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [hotelesData, habitacionesData, reserva] = await Promise.all([
          getHoteles(),
        ...
        setSelectedHotelId(reserva.hotelId);
        setSelectedHabitacionId(
          habitacionesData.find((h) => h.habitacionId === Number(reserva.idHabitacion))?.id || ''
        );

        if (reserva.huesped?.idUsuario) {
          try {
            const usuario = await getUsuarioById(Number(reserva.huesped.idUsuario));
            setDniInput(usuario.dni);
            setUsuarioEncontrado(usuario);
          } catch {
            setDniInput(reserva.huesped.idUsuario);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const reserva = await actualizarReserva(reservaId, {
  ...
  if (pageLoading) return <div style={{ padding: '20px' }}><p>Cargando datos...</p></div>;
